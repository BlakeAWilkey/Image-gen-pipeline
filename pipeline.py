...
class Visual_asset_pipeline(object):
    ...
    def _setup_logger(self):
        name = f"{self.client_id}_pipeline"
    ...
    def generate_sentence_embeddings(self):
        ...
        self.sentences, self.embeddings = u.embed_sentences(
            self.initial_notes,
            use_pca=True,
            save_fig=f"./graphs/embedding_principal_components{self.client_id}",
        )
        self.logger.info("Calculated sentence embeddings")
        return
    ...
    def compute_optimal_clusters(self):
        ...
        if self.embeddings.size == 0:
            self.logger.error(
                "Failed to compute optimal clusters, no available embeddings"
            )
            return

        max_clusters = len(self.embeddings)
        self.cluster_assignments = u.find_optimal_clusters(
            self.embeddings,
            max_clusters,
            f"./graphs/clustering_log_likelihoods_client{self.client_id}",
        )
        self.logger.info("Computed optimal clusters")

    def generate_image_prompts(
        self, model_dir="./model_cache/ollama", num_variations=3, random_clusters=False
    ):
        """
        ...
        if self.sentences.size == 0:
            self.logger.error("Failed to generate image prompts. No provided sentences")
            return
        elif self.cluster_assignments.size == 0:
            self.logger.error(
                "Failed to generate image prompts. No cluster assignments for sentences"
            )
            return
        elif self.cluster_assignments.shape[0] != self.sentences.shape[0]:
            self.logger.error(
                "Failed to generate image prompts. Number of Cluster assignments don't match the number of sentences"
            )
            return
        ...
        with tqdm(total=1, desc="Determining Graphic Deliverable") as progress:
            topic_result = u.generate_prompt(
                topic_prompt, model=self.llm_model, model_dir=model_dir
            )
            topic = topic_result
        ...
        if self.verbose:
            self.logger.info(
                f"Generated graphic deliverable type: '{base_image_prompt[:200]}' ..."
            )

        base_image_prompt += " "

        # Use all clusters to get one prompt
        for cluster_num in trange(
            0, np.max(self.cluster_assignments) + 1, desc="Generating Image Descriptors"
        ):
            ...
            condensed_phrase = (
                u.generate_prompt(
                    descriptor_preamble + formatted_descriptions,
            ...
            if self.verbose:
                self.logger.info(
                    f"Generated Descriptive Phrase: '{condensed_phrase[:200]}' ..."
                )

        image_prompts.append(
            (
                f'{self.client_id}-{datetime.now().strftime("%Y%m%d_%H%M%S")}-all',
                base_image_prompt,
            )
        )
        ...
        if random_clusters:
            for i in trange(0, num_variations, desc="Generating Prompt Variants"):
                ...
                image_prompts.append(
                    (
                        f'{self.client_id}-{datetime.now().strftime("%Y%m%d_%H%M%S")}-{"".join(str(selected))}',
                        variant,
                    )
                )
        ...
        for label, prompt in tqdm(image_prompts, desc="Refining Image Prompts"):
            revised_prompt = u.generate_prompt(
                refine_prompt + prompt, model=self.llm_model, model_dir=model_dir
            )
            self.add_image_prompt(label, revised_prompt)

        self.logger.info("Finished refining image prompts")

    def add_image_prompt(self, label, prompt):
        self.base_image_prompts.append((label, prompt))
        self.logger.info(f"Added '{prompt[:200]}...' to prompt queue")
    ...
    def generate_images(self):
        ...
        for label, prompt in tqdm(self.base_image_prompts, desc="Generating Images"):
            self.logger.info(f"Generating image with label {label}")
            u.generate_image(prompt, label)
            self.logger.info(f"Finished generating {label}")
